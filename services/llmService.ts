...
const GEMINI_INTERNAL_MODEL = 'gemini-3.5-flash';
...
const getStoredGeminiKey = (): string => {
    try {
        ...
        return typeof parsedKeys.gemini === 'string' ? parsedKeys.gemini.trim() : '';
    } catch (error) {
        return '';
    }
};

const getGeminiApiKey = (apiKeys?: ApiKeys): string => {
    return apiKeys?.gemini?.trim() || getStoredGeminiKey() || process.env.API_KEY || '';
};
...
export async function checkGeminiStatus(apiKey: string): Promise<{ ok: boolean; message: string }> {
    if (!apiKey || apiKey.length < 10) {
        return { ok: false, message: 'Gemini API key is missing or too short.' };
    }
    try {
        ...
        return { ok: false, message: 'Network error while validating Gemini key.' };
    }
}
...
async function* getProviderStream(
    ...
    switch (provider) {
        case 'gemini':
            yield* streamGemini(messages, fullSystemPrompt, config, apiKeys);
            break;
        case 'openai':
...
async function* streamGemini(
    ...
    const apiKey = getGeminiApiKey(apiKeys);
    if (!apiKey) {
...
export async function* streamAnalysis(messages: Message[], apiKeys?: ApiKeys): AsyncGenerator<string> {
    const apiKey = getGeminiApiKey(apiKeys);
    if (!apiKey) {
        throw new Error("Gemini API key is missing for analysis.");
    }
...
export async function* streamAdversarialSuggestion(
    ...
    const apiKey = getGeminiApiKey(apiKeys);
    if (!apiKey) {
        throw new Error("Gemini API key is missing for adversarial mode.");
    }
...
export async function generateAgentTools(
    ...
    try {
        const model = ai.getGenerativeModel({ model: GEMINI_INTERNAL_MODEL });
