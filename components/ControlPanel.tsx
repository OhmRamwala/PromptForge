...
import { checkOllamaStatus, checkOpenAIStatus, checkAnthropicStatus, checkGeminiStatus } from '../services/llmService';
...
const ControlPanel: React.FC<ControlPanelProps> = ({
    ...
    const [keyVisibility, setKeyVisibility] = useState({ gemini: false, openAI: false, ollama: false, anthropic: false });
    
    const [geminiStatus, setGeminiStatus] = useState<StatusState>({ checking: false, ok: null, message: '' });
    ...
    // Validation Effect for Gemini
    useEffect(() => {
        if (!apiKeys.gemini || apiKeys.gemini.length < 10) {
            setGeminiStatus({ checking: false, ok: null, message: '' });
            return;
        }
        const handler = setTimeout(async () => {
            setGeminiStatus({ checking: true, ok: null, message: 'Validating key...' });
            const status = await checkGeminiStatus(apiKeys.gemini.trim());
            setGeminiStatus({ checking: false, ok: status.ok, message: status.message });
        }, 500);
                ...
                return (
                    <div>
                        <div className="flex items-center justify-between mb-1">
                            <label htmlFor="gemini_key" className="block text-sm font-medium text-sentinel-text-secondary">Gemini API Key</label>
                            {renderStatusIndicator(geminiStatus)}
                        </div>
                        <div className="relative">
                            ...
                            <button
                                type="button"
                                onClick={() => setKeyVisibility(prev => ({ ...prev, gemini: !prev.gemini }))}
                            ...
                            >
                                {keyVisibility.gemini ? <EyeSlashedIcon className="h-4 w-4" /> : <EyeIcon className="h-4 w-4" />}
                            </button>
                        </div>
                        <p className="mt-1 text-[10px] text-sentinel-text-secondary">Saved locally in this browser. The .env key is still used as a fallback.</p>
                    </div>
                );
            case 'openai':
                return (
